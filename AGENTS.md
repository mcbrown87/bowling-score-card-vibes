...
## Testing Guidelines
Frontend tests use CRA's Jest + React Testing Library stack (see `bowling-scorecard/src/App.test.tsx` and `setupTests.ts`). Name files `*.test.tsx` alongside the component, focus on user flows (image upload, score correction), and run `npm test -- --watch` while developing. Backend currently lacks automated tests; add Vitest/Jest suites under `backend/src/__tests__` whenever you ship logic-heavy modules.
